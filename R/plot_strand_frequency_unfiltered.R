#' Frequency distribution: S1 and S2 (unfiltered)
#'
#' Shows the distribution of read depths for single-strand and double-strand
...
#' per strand after filtering. Useful for seeing what gets removed by filtering.
plot_strand_frequency_unfiltered <- function(result,
                                             outdir = NULL,
                                             sample_name = "sample",
                                             reads_max = 75,
  ...
  freq_dt <- unfiltered[reads >= 1 & reads <= reads_max,
                        .N,
  ...
  # If showing thresholds, extract minimum reads from filtered data
  if (show_thresholds) {
    s1_marks <- result$merged[Strand == "S1", .(Min_Reads = min(reads)), by = Pipeline]
    s2_marks <- result$merged[Strand == "S2", .(Min_Reads = min(reads)), by = Pipeline]
  }
  ...
  p_freq <- p_freq +
    ...
    ggplot2::scale_x_continuous(breaks = c(1, 25, 50, 75)) +
    ...
    ggplot2::labs(
      ...
      title = "Single-strand and Double-strand methylation sites across pipelines",
  ...
  if (!is.null(outdir)) {
    suffix <- if (show_thresholds) "_02Fig_Frequency_Unfiltered_with_Thresholds.pdf"
    else "_02Fig_Frequency_Unfiltered.pdf"
